...
export default function CartDrawer() {
  ...
  const upiLink = `upi://pay?${new URLSearchParams({
    pa: "9814996633.39@indianbank",
    pn: "Mann Organiculture",
    am: total.toFixed(2),
    cu: "INR",
  }).toString()}`;
    ...
    <>
      {itemCount > 0 && (
        <>
          <a
            href={upiLink}
            className="fixed bottom-40 right-5 z-40 inline-flex items-center gap-2 rounded-full bg-forest-900 px-5 py-3 text-[14px] font-semibold text-cream shadow-[0_8px_24px_rgba(15,47,33,0.24)] transition-transform hover:-translate-y-0.5 hover:bg-forest-700 sm:right-8"
            aria-label="Pay using UPI"
          >
            Pay
          </a>
          <button
